...
class WGAN_clip(keras.Model):
    """Wasserstein GAN whose critic weights are clipped to [-1, 1].

    The generator minimizes ``-mean(critic(fake))`` and the critic minimizes
    ``mean(critic(fake)) - mean(critic(real))``. Lipschitz continuity of the
    critic is enforced by clipping its trainable variables after each update.
    """

    def __init__(self, generator, discriminator, latent_generator,
                 real_examples, batch_size=200, disc_iter=2,
                 learning_rate=.005):
        """Initialize the WGAN and compile both sub models."""
        super().__init__()
    ...
    def train_step(self, useless_argument):
        """Run one training step and return the last critic and generator
        losses."""
