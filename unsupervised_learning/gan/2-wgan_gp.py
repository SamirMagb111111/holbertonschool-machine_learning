...
class WGAN_GP(keras.Model):
    """Wasserstein GAN whose critic is regularized with a gradient penalty.

    Instead of clipping the critic weights, the squared distance of the
    gradient norm to 1 is penalized on samples interpolated between the real
    and fake batches, which enforces the 1-Lipschitz constraint more softly.
    """

    def __init__(self, generator, discriminator, latent_generator,
                 real_examples, batch_size=200, disc_iter=2,
                 learning_rate=.005, lambda_gp=10):
        """Initialize the WGAN-GP and compile both sub models."""
        super().__init__()
    ...
    def get_interpolated_sample(self, real_sample, fake_sample):
        """Interpolate linearly between a real and a fake batch."""
        u = tf.random.uniform(self.scal_shape)
        return u * real_sample + (1 - u) * fake_sample

    def gradient_penalty(self, interpolated_sample):
        """Compute the gradient penalty for an interpolated batch."""
    ...
    def train_step(self, useless_argument):
        """Run one training step and return the critic loss, the generator
        loss and the gradient penalty."""
